...
function toActivityEventFromWorkflowEvent(
  ...
  return {
    ...
    title,
    detail,
    timestamp: String(row.timestamp || row.ts || data.timestamp || metadata.timestamp || new Date().toISOString()),
    stage: eventFamily === "plan" ? "plan" : "execute",
    status: eventType.includes("error") || eventType.includes("failed") ? "failed" : "info",
    snapshot_ref: String(row.snapshot_ref || data.snapshot_ref || metadata.snapshot_ref || "").trim() || undefined,
    scene_ref: String(row.scene_ref || data.scene_ref || metadata.scene_ref || "").trim() || undefined,
    graph_node_id:
      String(row.graph_node_id || data.graph_node_id || metadata.graph_node_id || "").trim() || undefined,
    data: resolvedData,
    metadata,
