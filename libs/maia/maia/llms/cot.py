...
from maia.base import BaseComponent, Document
...
class Thought(BaseComponent):
    ...
    ```python
    >> from maia.pipelines.cot import Thought
    >> thought = Thought(
...
class ManualSequentialChainOfThought(BaseComponent):
    ...
    `maia.pipelines.cot.Thought`. Please refer that section for
    ...
    **Create and run a chain of thought without "+" operator:**
    ...
    ```pycon
    >>> from maia.pipelines.cot import Thought, ManualSequentialChainOfThought
    >>> llm = LCAzureChatOpenAI(...)
    >>> thought1 = Thought(
    >>>    prompt="Word {word} in {language} is ",
    >>>    post_process=lambda string: {"translated": string},
    ...
    **Create and run a chain of thought without "+" operator:** Please refer the
    `maia.pipelines.cot.Thought` section for examples.
    ...
    llm: LLM = Param(help="The LLM model to use (base of maia.llms.BaseLLM)")
