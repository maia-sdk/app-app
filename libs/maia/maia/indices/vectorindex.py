...
from maia.base import BaseComponent, Document, RetrievedDocument
...
def _safe_cache_stem(raw_name: str) -> str:
    ...
    stem = Path(base).stem or Path(base).name or "chunk"
...
class VectorIndexing(BaseIndexing):
    ...
    def write_chunk_to_file(self, docs: list[Document]):
        ...
        if self.cache_dir:
            raw_file_name = docs[0].metadata.get("file_name")
            if not raw_file_name:
                return

            cache_dir = Path(self.cache_dir)
            cache_dir.mkdir(parents=True, exist_ok=True)
            file_stem = _safe_cache_stem(str(raw_file_name))
            ...
            for i in range(len(docs)):
                ...
                with open(
                    cache_dir / f"{file_stem}_{self.count_+i}.md",
                    "w",
                    encoding="utf-8",
                ) as f:
