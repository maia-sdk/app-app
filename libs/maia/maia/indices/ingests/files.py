...
from maia.base import BaseComponent, Document, Param
from maia.indices.extractors import BaseDocParser
from maia.indices.splitters import BaseSplitter, TokenSplitter
from maia.loaders import (
    AdobeReader,
...
KH_OCR_FILE_EXTRACTORS: dict[str, BaseReader] = {
...
class DocumentIngestor(BaseComponent):
    ...
    def _get_reader(self, input_files: list[str | Path]):
        ...
        extractor_pool = (
            KH_OCR_FILE_EXTRACTORS if self.pdf_mode == "ocr" else KH_DEFAULT_FILE_EXTRACTORS
        )
        file_extractors: dict[str, BaseReader] = {
            ext: reader for ext, reader in extractor_pool.items()
        }
        ...
        if self.pdf_mode == "normal":
            ...
            file_extractors[".pdf"] = OCRAugmentedPDFReader()
