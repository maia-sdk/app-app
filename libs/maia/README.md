...
Quick and easy AI components to build Maia
...
```shell
pip install maia@git+ssh://git@github.com/Cinnamon/maia.git
```
...
purposes only. Thus, it should not be used in the main source code (i.e. `maia/` and `tests/`), but can be used in `examples/`.
